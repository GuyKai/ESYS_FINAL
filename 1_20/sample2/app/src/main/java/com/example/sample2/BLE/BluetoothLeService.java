package com.example.sample2.BLE;
...
import com.example.sample2.Alarm.AlarmService;
import com.example.sample2.Alarm.ServiceContainer;
...
public class BluetoothLeService extends Service {
    ...
    public static final int CHARACTERISTIC_ACCELERO = 0 ;
    public static final int CHARACTERISTIC_NOTIFY = 1 ;
    public static final int SERVICE_LIGHT = 3;
    public static final int CHARACTERISTIC_LIGHT= 0 ;
    public static final int CHARACTERISTIC_LIGHT_I= 1 ;
    public static final int SERVICE_MOTOR= 4;
    public static final int CHARACTERISTIC_MOTOR= 0 ;

    public final static String ACTION_GATT_CONNECTED =
            "com.example.bluetoothexample.ACTION_GATT_CONNECTED";//已連接到GATT服務器
    public final static String ACTION_GATT_DISCONNECTED =
            "com.example.bluetoothexample.ACTION_GATT_DISCONNECTED";//未連接GATT服務器
    public final static String ACTION_GATT_SERVICES_DISCOVERED =
            "com.example.bluetoothexample.ACTION_GATT_SERVICES_DISCOVERED";//未發現GATT服務
    public final static String ACTION_DATA_AVAILABLE =
            "com.example.bluetoothexample.ACTION_DATA_AVAILABLE";//接收到來自設備的數據，可通過讀取或操作獲得
    public final static String EXTRA_DATA = "com.example.bluetoothexample.EXTRA_DATA"; //其他數據
    private boolean lockCharacteristicRead = false;//由於送執會觸發onCharacteristicRead並造成干擾，故做一個互鎖
    private boolean GattSet = false;
    ...
    public void onCreate() {
        ...
        filter.addAction("ALARM_SET");
        filter.addAction("ALARM_CANCEL");
    ...
    public void setGatt(List<BluetoothGattService> gattList){
        GattSet = true;
        mServiceContainer.setServiceInfo(gattList);
    }

    public ServiceInfo.CharacteristicInfo getGatt (int service,int characteristic){
        return mServiceContainer.getInfo(service,characteristic);
    }

    public boolean getGattSet(){
        return GattSet;
    }
    ...
    public boolean sendValue(String value, BluetoothGattCharacteristic characteristic) {
        try {
            ...
            BluetoothGattCharacteristic RxChar = characteristic;
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            //Log.d(TAG,"here");
            if (alarmReceived){
                if (byteArrayToHexStr(a).equals("01")){
                    Log.d(TAG,"awake");
                    if (active){
                        broadcastUpdate("AWAKE");
                    }

                }
                else if (byteArrayToHexStr(a).equals("00")){
                    Log.d(TAG,"sleep");
                    if (!active){
                        broadcastUpdate("SLEPT");
                    }
    ...
    public class mAlarmReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            else if ("ALARM_SET".equals(action)){
                Log.d(TAG, "notify");
                notify_on();
            }
            else if ("ALARM_CANCEL".equals(action)){
                Log.d(TAG, "cancel");
                alarmReceived = false;
                finish();
            }
            else if ("AWAKE".equals(action)){
                Log.d(TAG, "awake");
                awake();
            }
            else if ("SLEPT".equals(action)){
                Log.d(TAG, "slept");
                onAlertReceived();
            }
            ...
            else if ("LIGHT_UP".equals(action)){
                Log.d(TAG, "light up");
                lightup();
            }
        }
    ...
    public void accelero_on(){
        ...
        byte[] msg = {0x01};
        //String msg ="1";
        ServiceInfo.CharacteristicInfo info = getGatt(SELECTED_SERVICE,CHARACTERISTIC_ACCELERO);
        sendValue(msg,info.getCharacteristic());
    ...
    public void notify_on(){
        ...
        ServiceInfo.CharacteristicInfo info = getGatt(SELECTED_SERVICE,CHARACTERISTIC_NOTIFY);
        setCharacteristicNotification(info.getCharacteristic(),true);
    ...
    public void notify_off(){
        ...
        ServiceInfo.CharacteristicInfo info = getGatt(SELECTED_SERVICE,CHARACTERISTIC_NOTIFY);
        setCharacteristicNotification(info.getCharacteristic(),false);

    }

    public void light_on(){
        byte[] msg = {0x01};
        ServiceInfo.CharacteristicInfo info = getGatt(SERVICE_LIGHT,CHARACTERISTIC_LIGHT);
        sendValue(msg,info.getCharacteristic());

    }

    public void light_change(byte[] I ){
        byte[] msg = I;
        ServiceInfo.CharacteristicInfo info = getGatt(SERVICE_LIGHT,CHARACTERISTIC_LIGHT_I);
        sendValue(msg,info.getCharacteristic());
    ...
    public void motor_on(){
        byte[] msg = {0x01};
        ServiceInfo.CharacteristicInfo info = getGatt(SERVICE_MOTOR,CHARACTERISTIC_MOTOR);
        sendValue(msg,info.getCharacteristic());
